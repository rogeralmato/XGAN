...
class Xgan:
    def __init__(self, sess, args):
        ...
        self.dataset_dir_A = args.dataset_dir_A
        self.dataset_dir_B = args.dataset_dir_B
    ...
    def _build_model(self):
        ...
        self.dann_loss = sce_criterion(self.cdann_A, tf.zeros_like(self.cdann_A)) + sce_criterion(self.cdann_B,
                                                                                                  tf.ones_like(
                                                                                                      self.cdann_B))
        ...
        self.dis_gan_loss_fake_A = self.criterionGAN(self.discriminate_fake_sample_A,
                                                     tf.zeros_like(self.discriminate_fake_sample_A))
        ...
        self.dis_gan_loss_fake_B = self.criterionGAN(self.discriminate_fake_sample_B,
                                                     tf.zeros_like(self.discriminate_fake_sample_B))
        ...
        self.rec_loss_sum = tf.compat.v1.summary.scalar("rec_loss", self.rec_loss)
        self.dann_loss_sum = tf.compat.v1.summary.scalar("dann_loss", self.dann_loss)
        self.sem_loss_sum = tf.compat.v1.summary.scalar("sem_loss", self.sem_loss)
        self.gen_gan_loss_sum = tf.compat.v1.summary.scalar("gen_gan_loss", self.gen_gan_loss)
        self.gen_loss_sum = tf.compat.v1.summary.scalar("gen_loss", self.gen_loss)
        self.gen_sum = tf.compat.v1.summary.merge(
            [self.rec_loss_sum, self.dann_loss_sum, self.sem_loss_sum, self.gen_gan_loss_sum, self.gen_loss_sum])

        self.dis_gan_loss_sum = tf.compat.v1.summary.scalar("dis_gan_loss", self.dis_gan_loss)
        self.dis_loss_sum = tf.compat.v1.summary.scalar("dis_loss", self.dis_loss)
        self.dis_sum = tf.compat.v1.summary.merge([self.dis_gan_loss_sum, self.dis_loss_sum])
    ...
    def train(self, args):
        ...
        self.gen_optim = tf.compat.v1.train.AdamOptimizer(self.lr, beta1=args.beta1).minimize(self.gen_loss,
                                                                                              var_list=self.gen_vars)
        self.dis_optim = tf.compat.v1.train.AdamOptimizer(self.lr, beta1=args.beta1).minimize(self.dis_loss,
        ...
        for epoch in range(args.epoch):
            dataA = glob(self.dataset_dir_A + '/*.jpg') + glob((self.dataset_dir_A + '/*.png'))
            dataB = glob(self.dataset_dir_B + '/*.jpg') + glob((self.dataset_dir_B + '/*.png'))
            ...
            for idx in range(0, batch_idxs):
                ...
                counter += 1
                print((
                        "Epoch: [%2d] [%4d/%4d] rec_loss: %4.3f|dann_loss: %4.3f|sem_loss: %4.3f|gen_gan_loss: %4.3f|gen_loss: %4.3f|dis_loss: %4.3f" % (
    ...
    def test(self, args):
        ...
        if args.which_direction == 'AtoB':
            sample_files = glob(self.dataset_dir_A + '/*.jpg') + glob(self.dataset_dir_A + '/*.png')
        elif args.which_direction == 'BtoA':
            sample_files = glob(self.dataset_dir_B + '/*.jpg') + glob(self.dataset_dir_B + '/*.png')
        else:
