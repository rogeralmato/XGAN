...
def encoder(image, options, domain_name):
    ...
    with tf.compat.v1.variable_scope("encoder_" + domain_name, reuse=tf.compat.v1.AUTO_REUSE):
        # image is (batch_size x 64 x 64 x input_c_dim)
    ...
    with tf.compat.v1.variable_scope("encoder_sharing", reuse=tf.compat.v1.AUTO_REUSE):
...
def decoder(input, options, domain_name):
    ...
    with tf.compat.v1.variable_scope("decoder_sharing", reuse=tf.compat.v1.AUTO_REUSE):
    ...
    with tf.compat.v1.variable_scope("decoder_" + domain_name, reuse=tf.compat.v1.AUTO_REUSE):
...
def cdann(input):
    ...
    with tf.compat.v1.variable_scope("cdann", reuse=tf.compat.v1.AUTO_REUSE):
...
def euc_criterion(in_, target):
    return tf.compat.v1.losses.mean_squared_error(target, in_)
